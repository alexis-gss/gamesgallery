...
var C = {
    ...
    lazyLoad: function(){
        ...
        for (let i = 0 ; i < lazyImage.length ; i ++) {
            if(M.elementInViewport(lazyImage[i])){
                ...
                lazyImage[i].parentNode.classList.add("contentInView");
            }
            else {
                lazyImage[i].parentNode.classList.remove("contentInView");
            }
        }
        C.updateArrow();
    },
    ...
    updateArrow: function() {
        if(M.checkScrollTop())
            V.showArrow('v-hidden', 'v-visible');
        else {
...
var V = {
    ...
    init: function(){
        ...
        V.btnTheme = document.querySelector(".btn_theme");
        V.btnThemeDark = document.querySelector(".theme_dark");
        V.btnThemeLight = document.querySelector(".theme_light");
        V.btnTop = document.querySelector(".btn_top")
    ...
    search: function(){
        for (let i = 0 ; i < V.games.length ; i ++) {
            if(!V.games[i].innerText.toLowerCase().includes(V.searchInput.value.toLowerCase())){
    ...
    updateThemeBtn: function(){
        if(C.theme === 'dark') {
            V.btnThemeDark.classList.remove('theme_activated');
            V.btnThemeLight.classList.add('theme_activated');
        } else {
            V.btnThemeDark.classList.add('theme_activated');
            V.btnThemeLight.classList.remove('theme_activated');
        }
    },
