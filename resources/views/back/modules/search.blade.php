<form action="{{ request()->url() }}" enctype="multipart/form-data" id="filter" class="d-flex flex-row input-group pt-3">
    <input class="form-control" type="text" data-bs="tooltip" data-bs-placement="top"
        title="{{ __('filter.search_game') }}" placeholder="{{ __('filter.search_game') }}" id="filter" name="filter"
        value="{{ old('filter', $filter ?? '') }}" required>
